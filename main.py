...
for i in range(num_tests):
    ...
    # Test with max_completion_tokens=129
    reasoning_tokens, latency, output = query_openai(prompt, max_completion_tokens=129)
    if reasoning_tokens is not None:
        tokens_max_129.append(reasoning_tokens)
...
def plot_frequency_distribution(tokens_list_1, tokens_list_3):
    tokens_list_1 = [item for item in tokens_list_1 if item <= 192]
    tokens_list_3 = [item for item in tokens_list_3 if item <= 192]
    ...
    sns.scatterplot(x=unique_values_1, y=counts_1, s=100, marker='s', color='blue', label='No Max Completion Tokens')

    # Plot for max_completion_tokens=129
    sns.scatterplot(x=unique_values_3, y=counts_3, s=100, marker='o', color='red', label='Max Completion Tokens = 129')
    ...
    for i in range(len(unique_values_1)):
        plt.text(unique_values_1[i] + 1, counts_1[i] + 0.1, str(unique_values_1[i]), fontsize=10, ha='left')
    for i in range(len(unique_values_3)):
        plt.text(unique_values_3[i] + 1, counts_3[i] + 0.1, str(unique_values_3[i]), fontsize=10, ha='left')
...
plot_frequency_distribution(tokens_no_limit, tokens_max_129)

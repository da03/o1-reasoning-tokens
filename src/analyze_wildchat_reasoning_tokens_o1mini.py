...
def plot_frequency_distribution(tokens_list, a):
    ...
    plt.title("Distribution of Reasoning Tokens for o1-mini (Dashed Lines at Multiples of 64)", fontsize=16)
    plt.xlabel("Reasoning Token Count", fontsize=14)
    plt.ylabel("Frequency", fontsize=14)

    plt.tight_layout()  # Adjust plot to fit the figure area nicely
    plt.savefig(f'counts_o1mini_wildchat.png')

def main():
    reasoning_tokens_all = torch.load('data/reasoning_tokens_o1mini.pt')
